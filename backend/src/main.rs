...
//! The application is configured using the following environment variables, typically set via Docker Compose.
//!
//! - `DATABASE_URL`:
...
impl IntoResponse for AppError {
    fn into_response(self) -> Response {
        ...
        error!("Responding with error: status={}, message='{}', details='{:?}'", status, message, self);
...
impl JwksClient {
    ...
    async fn get_decoding_key(&self, kid: &str) -> Result<DecodingKey, AppError> {
        ...
        info!("JWKS Client: Key with kid '{}' not in cache. Fetching JWKS from URL: {}", kid, self.jwks_uri);
...
async fn auth_middleware(State(state): State<AppState>, request: Request, next: Next) -> Result<Response, AppError> {
        ...
        match general_purpose::URL_SAFE_NO_PAD.decode(payload_b64) {
            Ok(decoded_payload_bytes) => {
                if let Ok(payload_str) = String::from_utf8(decoded_payload_bytes) {
                    info!("Auth middleware: Decoded payload (for inspection): {}", payload_str);
                } else {
    ...
    info!(
        ...
        state.jwt_validation.iss,
        state.jwt_validation.aud,
        state.jwt_validation.validate_exp
    );

    // Decode the token and handle potential validation errors for better logging.
    let token_data = match decode::<Claims>(token, &decoding_key, &state.jwt_validation) {
        Ok(data) => {
            info!("Auth middleware: Token validated successfully with claims: {:?}", data.claims);
            data
        },
        Err(err) => {
            ...
            if let jsonwebtoken::errors::ErrorKind::InvalidIssuer = err.kind() {
                ...
                let unverified_claims = insecure_decode::<Claims>(token)
                    ...
                    .ok();
                let actual_issuer = unverified_claims.map(|c| c.iss).unwrap_or_else(|| "unknown".to_string());
                
                error!("Auth middleware: Invalid issuer. Expected one of: {:?}, but got: '{}'", state.jwt_validation.iss, actual_issuer);
            }
...
async fn get_fruits_handler(State(state): State<AppState>) -> Result<Json<Vec<Fruit>>, AppError> {
    ...
    let rows = client.query("SELECT id, name, origin, price FROM fruit ORDER BY id", &[]).await?;
    let fruits: Vec<Fruit> = rows.iter().map(|row| Fruit {
        id: row.get("id"),
        name: row.get("name"),
...
async fn get_fruit_by_id_handler(
    ...
    if let Some(row) = row_opt {
        ...
        Err(AppError::NotFound(format!("Fruit with id '{}' not found", fruit_id)))
    }
}
